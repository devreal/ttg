...
/**
 * Generator implementation adapted from https://github.com/lewissbaker/cppcoro
 */
...
namespace ttg {
  ...
  namespace detail
  {
    ...
    public:
      ...
      Keygen<T> get_return_object() noexcept;
  ...
  public:
    ...
    Keygen& operator=(Keygen other) noexcept
    {
      swap(other);
      return *this;
    }
    ...
    void swap(Keygen& other) noexcept
    {
  ...
  private:
    ...
    explicit Keygen(std::coroutine_handle<promise_type> coroutine) noexcept
      : m_coroutine(coroutine)
    {}
  ...
  {
    template<typename T>
    Keygen<T> generator_promise<T>::get_return_object() noexcept
    {
      using coroutine_handle = std::coroutine_handle<generator_promise<T>>;
      return Keygen<T>{ coroutine_handle::from_promise(*this) };
    }
  }
